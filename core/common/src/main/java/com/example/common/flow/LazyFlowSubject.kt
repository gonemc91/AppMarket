...
/**
 * Loader function foe [LazyFlowSubject]
 */
 ...
 * one subscribe starts collecting the flow returned by [listen] method.
 *
 * A value loader can be re-assigned with new one by using [newload] and [newAsyncLoader]
 * methods. In this case an old load is cancelled and replaced by a new one.
 *
 * Also it's possible to update the value directly be calling [updateWith] method.
 */
...
interface LazyFlowSubject<T> {
     ...
     * Value is loaded automatically when at least 1 subscribe starts
    ...
    /**
     * Updated the value immediately in a flow returned by [listen]
     */

    fun updatedWith(container: Container<T>)

    /**
     * Updated the value immediately in a flow returned by [listen] by
